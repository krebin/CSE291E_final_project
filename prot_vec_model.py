...
class ProtVecModel(nn.Module):

    def __init__(self):
        super().__init__()
        self.cnn_1d_3 = nn.Conv1d(in_channels=100, out_channels=100, stride=1, kernel_size=3, padding=1, bias=True)
        self.cnn_1d_5 = nn.Conv1d(in_channels=100, out_channels=100, stride=1, kernel_size=5, padding=2, bias=True)
        self.cnn_1d_1_1 = nn.Conv1d(in_channels=800, out_channels=500, stride=1, kernel_size=1, bias=True)
        self.cnn_1d_1_2 = nn.Conv1d(in_channels=1000, out_channels=500, stride=1, kernel_size=1, bias=True)

        self.gru_1 = nn.GRU(input_size=300, hidden_size=250, num_layers=1, batch_first=True, bidirectional=True)
